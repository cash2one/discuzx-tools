...
class Attachment(BasicBase):
    ...
    CREATE TABLE `bbs_attachment` (
      ...
      `key_name` VARCHAR(80) NOT NULL COMMENT '七牛文件名',
      `down_link` VARCHAR(150) NOT NULL COMMENT '下载地址',
      `plate` INT DEFAULT 0 COMMENT '版块',
      `status` INT DEFAULT 0 COMMENT '是否发帖(0:未传;1:已传;2:已发)',
      `author` VARCHAR(45) NULL COMMENT '所属用户',
      `create_datetime` timestamp NOT NULL COMMENT '添入时间',
      `upload_datetime` timestamp NOT NULL COMMENT '上传时间',
    ...
    key_name = Column(VARCHAR)
    down_link = Column(VARCHAR)
    plate = Column(INTEGER)
    status = Column(INTEGER)
    author = Column(VARCHAR)
    create_datetime = Column(TIMESTAMP)
    upload_datetime = Column(TIMESTAMP)

    def __init__(self, file_name, plate, author):
        """扫描文件目录记录入库.
        """
        ...
        self.plate = plate
        self.author = author
        self.create_datetime = time.time()
    ...
    def after_upload_action(self, key_name, down_link):
        ...
        self.status = 1
        self.upload_datetime = time.time()
...
class Thread(BasicBase):
    """
    CREATE TABLE `discuz_extend`.`bbs_thread` (
      `id` INT NOT NULL AUTO_INCREMENT COMMENT '自动编号',
